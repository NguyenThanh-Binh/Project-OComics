...
describe('<ClampText>', () => {
    test(' it should render properly', () => {
        const { container } = render(_jsx(ClampText, { rows: 3, children: innerText }));
        expect(container).not.toBeNull();
    });
    test(' it should render properly w/o # of rows', () => {
        const { container } = render(_jsx(ClampText, { children: innerText }));
        expect(container).not.toBeNull();
    });
    test(' it should expand properly', () => {
        const { container, getByTestId } = render(_jsx(ClampText, { expandable: true, rows: 1, children: innerText }));
